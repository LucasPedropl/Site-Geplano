...
const stripDeprecatedFields = (data: any) => {
	if (!data || typeof data !== 'object') return data;
	const cleaned: any = { ...data };

	if (cleaned.header && typeof cleaned.header === 'object') {
		const { nav_team: _removed, ...restHeader } = cleaned.header;
		cleaned.header = restHeader;
	}
	...
	if (isFirebaseInitialized && db) {
		try {
			...
			if (docSnap.exists()) {
				...
				const data = stripDeprecatedFields(
	...
	try {
		...
		if (stored) {
			...
			return stripDeprecatedFields(
				mergeWithDefaults(INITIAL_SITE_DATA, parsedStored)
			) as SiteData;
		}
	} catch (e) {
