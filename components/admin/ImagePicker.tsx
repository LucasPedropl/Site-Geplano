			...
			{label && (
				<label className="block text-sm font-bold text-gray-700 mb-2 uppercase tracking-wide flex items-center justify-between truncate">
					<div className="flex items-center gap-2">
						<ImageIcon
							size={16}
							className="text-geplano-green shrink-0"
						/>
						{label}
					</div>
					{!compact && (
						<div className="flex bg-gray-100 rounded p-1">
							<button
								...
								className={`px-2 py-1 text-[10px] font-bold rounded transition-colors ${
									inputType === 'upload'
										? 'bg-white shadow text-geplano-green'
				...
				{inputType === 'upload' && (
					<div className="relative group">
						...
						<button
							...
							className={`w-full border-2 border-dashed border-gray-300 bg-gray-50 hover:bg-green-50 hover:border-geplano-green rounded-lg flex items-center justify-center gap-2 transition-all ${
								compact ? 'py-2' : 'py-6'
							}`}
						>
							{isCompressing ? (
								<span className="text-geplano-green font-bold text-sm animate-pulse">
									Comprimindo...
								</span>
							) : (
								<>
									<Upload
										size={compact ? 14 : 20}
										className="text-gray-400 group-hover:text-geplano-green"
									/>
				...
				{inputType === 'link' && (
					<div className="relative w-full">
						...
						<input
							...
							placeholder={
								isBase64
									? '(Imagem carregada via upload)'
									: 'https://...'
							}
							disabled={isBase64}
							className={`w-full bg-white border border-gray-200 rounded-lg text-gray-800 outline-none focus:ring-2 focus:ring-geplano-green/20 focus:border-geplano-green transition-all pl-9 ${
