...
export const Solutions: React.FC<SolutionsProps> = ({ data }) => {
  ...
  const inactiveTabClass = "border-transparent text-gray-400 hover:text-gray-600";
        ...
        <Reveal>
            ...
            <div className="min-h-[300px] px-2">
                ...
                <div className="grid md:grid-cols-3 gap-8 animate-fade-in-up">
                    ...
                    <div key={idx} className="bg-white p-8 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-all duration-300">
                        <h3 className="text-xl font-bold text-geplano-green mb-4">{item.title}</h3>
                ...
                <div className="grid md:grid-cols-2 gap-8 animate-fade-in-up">
                    {data.content.advantages.map((item, idx) => (
                    <div key={idx} className="bg-white p-8 rounded-xl shadow-md hover:shadow-xl transition-shadow duration-300 text-left border-l-4 border-geplano-gold">
                        <h3 className="text-xl font-bold text-geplano-green mb-3">{item.title}</h3>
                        <p className="text-gray-600 leading-relaxed">{item.text}</p>
                    </div>
                    ))}
                </div>
                ...
                <div className="animate-fade-in-up">
                    ...
                    <p className="text-lg text-gray-700 mb-12 max-w-3xl mx-auto leading-relaxed" dangerouslySetInnerHTML={{ __html: data.content.delivery.subtitle.replace('GESTÃO INTEGRADA', '<strong class="text-geplano-gold">GESTÃO INTEGRADA</strong>') }} />
                    
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-y-10 gap-x-6 text-left">
                    {data.content.delivery.steps.map((step, idx) => (
                        <div key={idx} className="relative pl-14 md:pl-0 md:text-center group">
                        ...
                        <p className="text-sm font-bold text-gray-800 uppercase leading-tight relative z-10">{step}</p>
                        </div>
                    ))}
                    </div>
                </div>
